...
$('#apply-job').on('click', function () {
    $("#apply-job option[value='请选择职称']").remove()
...
//                    user = data.userid
                    $.ajax({
                        ...
                        type: "get",
                        dataType: 'json',
                        url: url + "CCApplyWBS.asmx/WBS_GetDDUserInfo?userid=00449",
                        success: function (data) {
                            ...
                            $.ajax({
                                ...
                                type: "get",
                                dataType: 'json',
                                url: 'http://120.27.217.7:7001/CCApplyWBS.asmx/WBS_GetDepListByUserID?userid=00449',
                                success: function (data) {
                                    DepID = data.data;
                                    //console.log(data)
                                }
...
$('#plcItem').on('click', function () {
    ...
    $.ajax({
        ...
        url: url + "CCApplyWBS.asmx/WBS_GetProInfo",
        success: function (data) {
...
$('#sub').on('click', function () {
    //判断是否输入必填项
    var JFtypes = document.getElementsByName('JFtype');
    var bool = false;
    for (var i = 0; i < JFtypes.length; i++) {
        if (JFtypes[i].checked == true) {
            bool = true;
        }
    }
    //判断是否继教
    var objc = document.getElementsByName('IsJJ');
    var chObJ = null;
    ...
    if ($('#jobNum').val() == '') {
        ...
        tatle('请输入手机号码')
        return
    } else if ($('#apply-SFZ').val().length != 18 && $('#apply-SFZ').val() != '') {
        tatle('身份证号为18位')
        return
    } else if ($('#plcItem').text() == '预算项目选择') {
        tatle('请选择预算项目')
        return
    ...
    for (var i = 0; i < childrens.length; i++) {
        ...
        $.ajax({
            url: url + "CCHandle.ashx?operKind=ReturnSaveImg",
            data: formData,
            processData: false,
            contentType: false,
            async: false,
    ...
    obj.SFZ = document.getElementById("apply-SFZ").value;
    obj.ZCName = job;
    obj.ZWName = title;
    ...
    for (var i = 0; i < JFtypes.length; i++) {
        if (JFtypes[i].checked == true) {
            obj.ProjectType = JFtypes[i].parentNode.previousElementSibling.innerText;
            obj.ProjectTypeID = JFtypes[i].value;
        }
    }

    var o = [];
    var flows_pre = document.getElementById("flows_pre").children;
    for (var i = 0; i < flows_pre.length; i++) {
        o.push({
            userid: JSON.parse(flows_pre[i].getAttribute("data-all")).userid,
            jobnumber: JSON.parse(flows_pre[i].getAttribute("data-all")).jobnumber,
            name: JSON.parse(flows_pre[i].getAttribute("data-all")).name,
            depid: JSON.parse(flows_pre[i].getAttribute("data-all")).department[0]
        }
            );
    }
    ...
    $.ajax({
        type: "post",
        dataType: 'json',
        data: { param: JSON.stringify(obj), paramSTPsn: JSON.stringify(o) },
        url: url + "CCApplyWBS.asmx/WBS_SaveCCInfo",
        success: function (data) {
            //alert("提交成功");
            if (data.success == 0) {
                layer.open({
