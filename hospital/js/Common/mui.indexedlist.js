...
(function ($, window, document) {
    ...
    var IndexedList = $.IndexedList = $.Class.extend({
        ...
        init: function (holder, options) {
            var self = this;
            mui.ajax(url+'CCApplyWBS.asmx/WBS_GetCCPlace ', {
                ...
                success: function (data) {
                    ...
                    for (var i = 0; i < da.length; i++) {
                        //html += '<li  data-Id="' + da[1].AutoID + '" data-value="' + temp + '" data-tags="' + temp + '" class="mui-table-view-cell mui-indexed-list-item">' + da[1].DName + '</li>';
                        console.log(data[i])
                        if (da[i].DCode == temp) {
